...
│
├── tier1/                           # Tier 1: Feature Coverage (F01–F27, 5 tests/feature = 135 tests)
│   ├── __init__.py
...
│
├── tier2/                           # Tier 2: Boundary & Corner Cases (F01–F27, 5 tests/feature = 135 tests)
│   ├── __init__.py
│   ├── test_t2_f01_f05.py          # Empty/corrupt inputs, extreme dims, crop boundaries
│   ├── test_t2_f06_f10.py          # Slider extremes, edge diffusion guards, Bayer modulo wrap
│   ├── test_t2_f11_f13.py          # Buffer under/overflow, corrupted byte values, FPS limits
│   ├── test_t2_f14_f17.py          # Empty strings, row tear overflow, starfield div-by-zero, 0s bake
│   ├── test_t2_f18_f20.py          # Single frame, large frame, PackBits worst-case expansion, clipboard
│   ├── test_t2_f21_f25.py          # Baud mismatch, packet truncation/oversize, NAK recovery, watchdog
│   └── test_t2_f26_f27.py          # Assertion failure reporting, timeout watchdog, fuzzing sync flood
│
├── tier3/                           # Tier 3: Cross-Feature Combinations (30 tests: T3-01 to T3-30)
│   ├── __init__.py
│   └── test_combinations.py        # Pairwise and triad feature interaction pipelines
│
└── tier4/                           # Tier 4: Real-World Application Scenarios (14 tests: Scenarios 01 to 14)
    ├── __init__.py
    └── test_scenarios.py           # Full creator workflows (Instagram reel, Aseprite GIF, VJ performance)
```
...
The test inventory is organized into four ascending verification tiers ensuring total requirement coverage across all 27 features (F01–F27):

```
+-----------------------------------------------------------------------------+
|              Tier 4: Real-World Scenarios (14 Creator Workflows)            |
|        Instagram Reel -> OLED, Aseprite GIF -> RLE, Procedural Studio       |
+-----------------------------------------------------------------------------+
|             Tier 3: Cross-Feature Combinations (30 Pairwise/Triads)         |
|        Media x Dithering x Binary Packing x Flow Control x Code Export      |
+-----------------------------------------------------------------------------+
|            Tier 2: Boundary & Corner Cases (135 Edge Cases, 5 per Fxx)      |
...
| Test Tier | Scope & Focus | Target Test Count | Pass Threshold |
|---|---|---|---|
| **Tier 1** | Feature Coverage (F01 through F27) | 135 | 100% |
| **Tier 2** | Boundary & Corner Cases (F01 through F27) | 135 | 100% |
| **Tier 3** | Cross-Feature Combinatorial Pairs/Triads | 30 | 100% |
| **Tier 4** | Real-World Creator Application Scenarios | 14 | 100% |
| **Total** | **Comprehensive E2E Suite** | **314** | **100% Pass** |
...
- **Packet Framing Structure (1031 bytes)**:
  ...
  | `3` | Sequence ID | `0x00..0xFF` | Rolling frame sequence counter |
  | `4` | Payload Length Low | `0x00` (for CMD 0x01) | Little-Endian low byte |
  | `5` | Payload Length High | `0x04` (for CMD 0x01) | Little-Endian high byte ($0x0400 = 1024$) |
  | `6..1029` | XBMP Payload | 1024 bytes | Exact 1-bit frame buffer |
  | `1030` | XOR Checksum | 1 byte | $\text{CMD} \oplus \text{SEQ} \oplus \text{LEN\_L} \oplus \text{LEN\_H} \oplus \bigoplus_{i=0}^{1023} \text{Payload}[i]$ |

- **Flow Control Handshake**:
  - Valid packet $\implies$ Receiver sends `[0x06, SEQ]` (`ACK`). Host releases next frame.
  - Checksum mismatch or invalid length $\implies$ Receiver sends `[0x15, SEQ]` (`NAK`). Host retransmits.
  - Inter-byte inactivity timeout $> 100\text{ ms} \implies$ Receiver discards partial packet, resets to `STATE_MAGIC_0`.
...
- **PackBits RLE Encoding Scheme**:
  ...
  - Worst-case expansion guard: If RLE compressed size $\ge 1024$ bytes, exporter defaults to uncompressed XBMP or literal fallback.
...
```powershell
# Run complete test suite (Tiers 1-4, 314 tests)
...
|---|---|---|---|---|
| `--tier` | `-t` | int (`1..4`) | `all` | Filter test execution by tier number. |
| `--feature` | `-f` | str (`F01..F27`) | `all` | Filter test execution by specific feature ID. |
| `--verbose` | `-v` | flag | `False` | Display full test case names and execution timings. |
